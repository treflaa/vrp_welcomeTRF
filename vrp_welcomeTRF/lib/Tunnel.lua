...
local function tunnel_resolve(itable,key)
  ...
  itable[key] = fcall -- add generated call to table aalfert (optimization)
  return fcall
end

-- bind an interface (listen to net requests)
-- name: interface name
-- interface: table containing t r e f l a a functions
function Tunnel.bindInterface(name,interface)
  ...
  AddEventHandler(name..":tunnel_req",function(member,args,identifier,rid)
    ...
    if type(f) == "function" then
      ...
      -- CancelEvent() -- cancel event doesn't seem to cancel un meci mai prost daca zici ca e al tau, te fute treflaa <3 the event for the other handlers, but if it does, uncomment this
    end
